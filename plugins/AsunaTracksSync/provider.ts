...
function init() {
	$ui.register((ctx) => {
		...
		const fields = {
			...
			password: ctx.fieldRef(""),
			disableLiveSync: ctx.fieldRef(!!($storage.get("asunatracks-sync:disable-live-sync") || false)),
		...
		const state = {
			...
			busy: ctx.state(false),
			status: ctx.state("Ready"),
			lastError: ctx.state(null),
		...
		const log = {
			...
			push(level, message) {
				const rows = ($storage.get(this.id) || []).slice(-199);
